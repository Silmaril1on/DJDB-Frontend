...
import { forChildrenX, forParent, zoomIn } from "../animations/motionValues";
import Link from "next/link";

const Logo = ({ className }) => {
  const letters = ["D", "J", "D", "B"];
  return (
    <Link href="/">
      <motion.div
        variants={forParent}
      ...
      >
        ...
        <motion.div
          variants={forParent}
          className={`flex-center absolute inset-0 overflow-hidden text-black font-bold lg:hover:scale-110 duration-300 ${className}`}
        >
          {letters.map((item, index) => {
            return (
              <motion.h1 variants={forChildrenX} key={index}>
                {item}
              </motion.h1>
            );
          })}
